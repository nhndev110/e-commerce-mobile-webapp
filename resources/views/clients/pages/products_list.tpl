{strip}
  ...
  {block name=head_tags}
    <link
      rel="stylesheet"
      href="/resources/assets/client/css/products.css?{time()}"
    >
  {/block}
  {$products_list = $data['products_list']}
  {$manufacturers_list = $data['manufacturers_list']}
          ...
          {foreach from=$products_list item=$product}
            ...
            <article class="product-info">
              <a
                class="product__link"
                title="{$product_name}"
                href="/products/{$product_name|regex_replace:"/[^-\w\d{$str_vietnamese}]+/"
        :"-"}-{$product['id']}"
              >
                <div class="product__image">
                  <img
                    class="product__img"
                    src="/resources/assets/client/image_products/{$product['image']}"
                    alt="{$product_name}"
                    title="{$product_name}"
                  />
                </div>
